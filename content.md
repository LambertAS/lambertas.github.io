...
Everything below is the actual copy for the site. Text in `[[ double brackets ]]`
is a decision I need to make myself before publishing. Text in `<!-- comments -->`
is a note to me and must never appear on the page.
...
<!-- Do not use a job-title-only tagline like "Aspiring Data Analyst". It says
     nothing and every entry-level portfolio has one. -->
...
unit. The org-unit mapping was the hard part. Reporting structures do not match
the shape of the data export, and getting responses attributed to the right unit
...
<!-- Keep this at the level of technique and scale. No internal figures, no survey
     results, no unit names, no employee data. This is a hard rule. -->
...
describe the same problem in incompatible vocabulary, and most of the value in
those meetings came from noticing when agreement was only apparent. Translating
what HR described into functional specifications that IT could estimate against
was the actual deliverable, not the meeting.
...
UAT is unglamorous and it is where requirements documents get tested against
reality. Several of the defects traced back to specification language that had
seemed unambiguous when written.

### HR service ticket SLA dashboard
**PT Lion Super Indo, 2026 · Power BI**

KPI reporting on service ticket SLA compliance, replacing a report that had been
compiled by hand each cycle.

### Performance Management System prototypes
**PT Lion Super Indo, 2026 · Figma**

Wireframes and interactive prototypes, iterated with HR stakeholders before
development started. Cheaper to find out a screen is wrong in Figma than after
it is built.
...
model that predicts "no stroke" for every patient scores about 95% accuracy and is
completely useless, which is the practical problem the paper addresses.
...
The calibration step mattered. Resampling changes the base rate the model sees, so
predicted probabilities stop meaning what they appear to mean unless you check.
...
Implementing it rather than importing it was the point. Metaheuristics are easy to
describe and easy to get subtly wrong, and writing it out surfaced parameter
...
[[ TODO: this section stays deleted until the project actually exists. ]]

<!-- Do not publish an empty "coming soon" section. It reads worse than not having
     the section at all. Build the public-data project first, then write this. -->
...
<!-- Phone number deliberately omitted. A public page with a number on it collects
     spam permanently and cannot be undone once indexed. -->

<!-- No contact form. A form needs a backend or a third-party service, and both are
     more failure surface than a mailto link. -->
